...
class LibraryAdapter(private var mList: List<Book>) : RecyclerView.Adapter<LibraryAdapter.ViewHolder>() {
    ...
    fun getBookTitle(position: Int):String{
        return mList.get(position).title;
    }

    fun getBookCover(position: Int):String{
        return mList.get(position).cover;
    }

    fun getBookPrice(position: Int):String{
        return mList.get(position).price;
    }
}